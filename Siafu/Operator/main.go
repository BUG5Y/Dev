...
func createTable(treeView *gtk.TreeView, store *gtk.ListStore) {
    ...
    for i, columnName := range columns {
        ...
        column, err := gtk.TreeViewColumnNewWithAttribute(columnName, renderer, "text", i)
        if err != nil {
...
func setupTab(notebook *gtk.Notebook, vbox *gtk.Box, win *gtk.Window, cmdPlaceHolder string, tabName string, paned *gtk.Paned) {
    ...
    tabWidget, button  := createTabWidget(tabLabel, notebook, paned, win, vbox)
...
// const nilbutton = &{{{{{0x0000000000}}}} 0x0000000000}
...
func createTabWidget(label *gtk.Label, notebook *gtk.Notebook, paned *gtk.Paned, win *gtk.Window, vbox *gtk.Box) (*gtk.Box, *gtk.Button) {
...
func createFileMenu(win *gtk.Window, notebook *gtk.Notebook, vbox *gtk.Box, cmdPlaceHolder string, tabName string, paned *gtk.Paned) *gtk.MenuItem {
    ...
    // "New Tab"
    newTabItem, _ := gtk.MenuItemNewWithLabel("Open Console")
...
var indicator = "<span>[" + "<span foreground=\"" + grayblue + "\">+</span>" + "]</span>"
...
func handleAlerts(output string, buffer *gtk.TextBuffer, warn bool){ // Inserts to CMD console
    ...
    alertindicator := "<span foreground=\"" + alertcolor + "\">[</span>" + "<span foreground=\"" + grayblue + "\">+</span>" + "<span foreground=\"" + alertcolor + "\">]</span> "
...
func insertLogMarkup(Text string) { // Inserts to log
    ...
    formattedTime := "<span foreground=\"" + grayblue + "\" font=\"Cantarell Italic Light 1 `wght`=200\">" + currentTime.Format("2006-01-02 15:04:05") + "</span>"
